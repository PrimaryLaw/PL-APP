...
const TopBar = ({ userId, chatId }: Props) => {
  const handleNewChat = async () => {
    await axios.post<Message[]>("/api/delete-messages", {
      chatId,
      userId
    });
    ...
    <nav className="relative flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all bg-mainGrey shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" navbar-main="" navbar-scroll="true">
      ...
      <div className="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
        ...
        <div className="flex items-center mt-2 grow sm:mt-0 sm:mr-6 md:mr-0 lg:flex lg:basis-auto">
          <button onClick={handleNewChat} className=" flex items-center md:ml-auto px-4 relative flex flex-col min-w-0 break-words bg-transparend rounded-lg border border-mainGreen shadow-soft-xl rounded-2xl bg-clip-border">
